      ...
      <Container size="lg">
        <div className="py-8 md:py-12">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-bold text-black tracking-tight">הכתובות שלי</h1>
            <Button size="sm"><Plus className="h-4 w-4" />הוסף כתובת</Button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {MOCK_ADDRESSES.map((addr) => (
              <div key={addr.id} className="bg-white rounded-xl border border-border p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button className="p-1.5 text-gray-400 hover:text-black transition-colors"><Pencil className="h-4 w-4" /></button>
                ...
                {!addr.isDefault && <button className="mt-4 text-sm text-black hover:text-muted font-medium transition-colors">הגדר כברירת מחדל</button>}
              </div>
            ))}
          </div>
        </div>
