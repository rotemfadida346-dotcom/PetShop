    ...
    <div className="bg-surface min-h-screen">
      <Container size="lg">
        <div className="py-8 md:py-12">
          ...
          <h1 className="text-3xl font-bold text-text-primary tracking-tight mb-8">המנויים שלי</h1>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <RefreshCw className="h-5 w-5 text-text-primary" />
                        <h2 className="font-semibold text-text-primary">{sub.productName}</h2>
                        <Badge variant={status.variant}>{status.label}</Badge>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                        <div><p className="text-xs text-text-secondary uppercase tracking-wide">תדירות</p><p className="text-sm font-medium text-text-primary mt-1">{getSubscriptionIntervalLabel(sub.intervalWeeks)}</p></div>
                        <div><p className="text-xs text-text-secondary uppercase tracking-wide">מחיר</p><p className="text-sm font-medium text-text-primary mt-1">{formatPrice(sub.pricePerDelivery)} <span className="text-xs text-text-secondary line-through">{formatPrice(sub.originalPrice)}</span></p></div>
                        <div><p className="text-xs text-text-secondary uppercase tracking-wide">חיסכון</p><p className="text-sm font-medium text-emerald-600 mt-1">{formatPrice(savings)}/משלוח</p></div>
                        <div><p className="text-xs text-text-secondary uppercase tracking-wide">משלוח הבא</p><p className="text-sm font-medium text-text-primary mt-1 flex items-center gap-1"><Calendar className="h-3.5 w-3.5" />{new Date(sub.nextDelivery).toLocaleDateString("he-IL", { month: "short", day: "numeric" })}</p></div>
                      </div>
                    </div>
          ...
          <div className="mt-8 bg-surface-hover border border-border rounded-xl p-6">
            <h3 className="font-semibold text-text-primary mb-2">יתרונות המנוי</h3>
