...
export const metadata: Metadata = { title: "חנות", description: "עיינו בקולקציה המלאה שלנו של מזון פרימיום לכלבים וחתולים, חטיפים, חול ועוד." };
...
export default async function ShopPage({ searchParams }: ShopPageProps) {
  ...
  try {
    ...
    products = result.products; totalPages = result.totalPages; total = result.total;
  } catch {
  ...
  return (
    <div className="bg-background min-h-screen">
      <div className={`border-b border-gray-200 ${isDog ? "bg-gradient-to-b from-dog-50 to-background" : isCat ? "bg-gradient-to-b from-cat-50 to-background" : "bg-gradient-to-b from-accent-50 to-background"}`}>
        <Container>
          <div className="py-10 md:py-14">
            <h1 className="text-heading-xl text-textPrimary">{title}</h1>
            <p className="mt-2 text-textMuted text-body-lg">{total} מוצרים במגוון קטגוריות</p>
          </div>
        </Container>
      </div>
      <Container>
        <div className="py-8 md:py-10">
          ...
          {products.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <h2 className="text-xl font-bold text-textPrimary mb-2">לא נמצאו מוצרים</h2>
              <p className="text-textSecondary">נסו לשנות את הסינון או עיינו בכל המוצרים.</p>
            </div>
          )}
        </div>
      </Container>
